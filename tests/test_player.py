...
def test_mpv_available():
    with patch("imj.player.shutil.which", return_value="/usr/bin/mpv"):
        assert player.mpv_available() is True
    with patch("imj.player.shutil.which", return_value=None):
